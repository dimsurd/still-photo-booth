...
export default function WebcamCapture({ setIsFlashing }: WebcamCaptureProps) {
  ...
  const flashAndCapture = useCallback(() => {
    setIsFlashing(true);
  ...
  }, [captures, isCapturing, flashAndCapture]);
    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div className="mt-4 flex gap-2">
          ...
          >
            {captures.filter(Boolean).length >= 4
              ? "Max Reached"
              : "Take Photo"}
          </button>
          <button
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="grid grid-cols-2 gap-4 h-fit">
          {captures.map((src, index) => (
            <div
              key={index}
              className="relative w-28 h-28 bg-gray-100 border rounded-md flex items-center justify-center overflow-hidden"
            >
              {src ? (
                <>
                  <Image
